...
const Chat = ({ offerId, factoryAddress, closeChat }) => {
  ...
  useEffect(() => {
    if (sendTofactoryCode !== null) {
      fetchMessages(); // Fetch messages after factory code is set
    }
  }, [sendTofactoryCode]); // Depend on sendTofactoryCode
  ...
  const fetchMessages = () => {
        ...
        onValue(messagesRef, (snapshot) => {
          ...
          for (let key in data) {
            if (checkIfChatBetweenThem(data[key], myFactoryCode, sendTofactoryCode) || 
                checkIfChatBetweenThem(data[key], sendTofactoryCode, myFactoryCode) || 
  ...
  const handleSendMessage = () => {
    ...
    const message = {
      ...
      to: sendTofactoryCode,
      from: Number(savedFactoryCode)
    };
